...
func (s *TaskService) TransitionState(ctx context.Context, actor app.Actor, taskID uuid.UUID, newState domain.TaskState, opts TaskTransitionOptions) (domain.MaintenanceTask, error) {
	...
	ctxTransition := domain.TaskTransitionContext{
		...
		AllowEarlyCompletion:  opts.AllowEarlyCompletion && (actor.Role == domain.RoleScheduler || actor.Role == domain.RoleAdmin || actor.Role == domain.RoleTenantAdmin),
		AllowLateCancel:       opts.AllowLateCancel && (actor.Role == domain.RoleScheduler || actor.Role == domain.RoleAdmin || actor.Role == domain.RoleTenantAdmin),
